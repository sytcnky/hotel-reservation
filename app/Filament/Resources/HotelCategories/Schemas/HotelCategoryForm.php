                                ...
                                ->schema([
                                    TextInput::make("name.$locale")
                                        ...
                                        ->afterStateUpdated(function (Set $set, Get $get, ?string $state, ?string $old) use ($locale) {
                                            ...
                                            // Slug boşsa ya da eski başlıktan türetilmişse → yeni başlığa göre güncelle
                                    ...
                                    TextInput::make("slug.$locale")
                                        ...
                                        ->maxLength(255)
                                        ->live(debounce: 300)
                                        // Kullanıcı yazarken TR/boşluk vb. her zaman normalize et
                                        ->afterStateUpdated(function (Set $set, ?string $state) use ($locale) {
                                            $set("slug.$locale", Str::slug($state ?? ''));
                                        })
                                        // Kayda geçmeden önce de normalize (ek güvenlik)
